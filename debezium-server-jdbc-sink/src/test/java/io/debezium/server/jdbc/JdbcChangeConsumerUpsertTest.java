...
@TestProfile(JdbcChangeConsumerUpsertTest.JdbcChangeConsumerUpsertTestProfile.class)
public class JdbcChangeConsumerUpsertTest extends BaseDbTest {
  @Inject
  ...
  public void testSimpleUpsert() throws Exception {
    ...
    Dataset<Row> ds = getPGTableData("inventory.customers_upsert");
    ...
    ds = getPGTableData("inventory.customers_upsert");
    ds.show();
  ...
  public void testSimpleUpsertCompositeKey() throws Exception {
    ...
    Dataset<Row> ds = getPGTableData("inventory.customers_upsert_compositekey");
    ds.show();
    ...
    consumer.handleBatch(records, TestUtil.getCommitter());
    ds = getPGTableData("inventory.customers_upsert_compositekey");
    ds.show();
    Assertions.assertEquals(ds.count(), 2);
  ...
  public void testSimpleUpsertNoKey() throws Exception {
    ...
    consumer.handleBatch(records, TestUtil.getCommitter());
    Dataset<Row> ds = getPGTableData("inventory.customers_upsert_nokey");
    ds.show();
    Assertions.assertEquals(ds.count(), 3);
    ...
    consumer.handleBatch(records, TestUtil.getCommitter());
    ds = getPGTableData("inventory.customers_upsert_nokey");
    ds.show();
    Assertions.assertEquals(ds.count(), 6);
    Assertions.assertEquals(ds.where("id = 1 AND __op= 'c' AND first_name= 'user2'").count(), 2);
  }

  public static class JdbcChangeConsumerUpsertTestProfile implements QuarkusTestProfile {
